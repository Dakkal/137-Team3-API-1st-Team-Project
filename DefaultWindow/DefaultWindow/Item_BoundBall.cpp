#include "stdafx.h"
#include "Item_BoundBall.h"
#include "Scene.h"
#include "SceneMgr.h"
#include "EventFunc.h"
#include "Player.h"

CItem_BoundBall::CItem_BoundBall()
{
}
...
void CItem_BoundBall::Initialize()
{
	m_tInfo.fCX = 30.f;
	m_tInfo.fCY = 30.f;
...
void CItem_BoundBall::Late_Update()
{
	if (WINCY <= m_tRect.bottom)
		m_bDead;
}

void CItem_BoundBall::Render(HDC hDC)
{
	Rectangle(hDC, m_tRect.left, m_tRect.top, m_tRect.right, m_tRect.bottom);
	TCHAR szName[32] = L"Æ¨±â´Â °ø";
	TextOut(hDC, m_tInfo.fX - 30.f, m_tInfo.fY + 20.f, szName, lstrlen(szName));
}

void CItem_BoundBall::Release()
{
}

void CItem_BoundBall::OnCollision(CObj * _pObj)
{
	if (m_bCollision)
	{
...
void CItem_BoundBall::Use_Item(CObj * _pObj)
{
	if (_pObj->GetObjType() == OBJECT_TYPE::PLAYER)
	{
